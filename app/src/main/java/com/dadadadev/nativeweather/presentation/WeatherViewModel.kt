        ...
        viewModelScope.launch {
            ...
            locationTracker.getCurrentLocation()?.let {location ->
                when(val result = repository.getWeatherData(location.latitude, location.latitude)) {
                    ...
                    is Resource.Error -> {
                        state.copy(weatherInfo = null, isLoading = false, error = result.message)
                    }
                }
            } ?: kotlin.run {
                state = state.copy(isLoading = false, weatherInfo = null, error = "Couldn't retrieve location. Make sure to grant permission or enable GPS.")
            }
        }
    }
}